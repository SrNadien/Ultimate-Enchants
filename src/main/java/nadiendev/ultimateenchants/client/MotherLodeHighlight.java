...
/**
 * Puramente client-side. No usa entidades ni sync con el server: cada cliente
 * escanea sus propios chunks ya cargados, igual que hace el mod de X-ray de
 * referencia (Controller.requestBlockFinder + RenderEnqueue.blockFinder).
 *
 * El escaneo corre en el executor de background de Minecraft para no trabar
 * el render thread, y solo se dispara mientras el jugador está efectivamente
 * bloqueando con el escudo encantado (igual que la condición vieja en
 * EnchantmentEventHandler, pero ahora sin ArmorStands).
 */
public final class MotherLodeHighlight {
    ...
    /** Leído por MotherLodeRenderer cada frame. Thread-safe: se reemplaza atómicamente. */
